...
function GestionInstructores() {
  ...
  const abrirEditar = (instructor) => {
    setEditandoId(instructor.id);
    setNombreCompleto(instructor.nombreCompleto);
    setEspecialidad(instructor.especialidad);
    setTurno(instructor.turno);
    setEstaActivo(instructor.estaActivo);
    setMensaje('');
    setModalAbierto(true);
  };
  ...
  const guardarInstructor = async (e) => {
    ...
    const datos = editandoId
      ? { id: editandoId, nombreCompleto, especialidad, turno, estaActivo }
      : { nombreCompleto, especialidad, turno };

    try {
      const url = editandoId
        ? `http://localhost:5027/api/Instructores/${editandoId}`
        : 'http://localhost:5027/api/Instructores';
      ...
      if (response.ok) {
        setMensaje(editandoId ? '¡Instructor actualizado con éxito!' : '¡Instructor registrado con éxito!');
        setTimeout(() => {
  ...
  const eliminarInstructor = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5027/api/Instructores/${id}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        alert('Ocurrió un error al intentar eliminar al instructor.');
      }
    } catch (err) {
    ...
    <div className="recepcion-container">
      <nav className="navbar-miembro">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div className="user-role">Panel de Instructores</div>
          <button onClick={() => navigate('/gestion-recepcion')} style={{ background: 'none', border: '1px solid #aa3bff', color: '#aa3bff', padding: '6px 16px', borderRadius: '6px', cursor: 'pointer' }}>
            Socios
          </button>
            ...
            <tbody>
              {instructoresFiltrados.map(i => (
                <tr key={i.id}>
                  ...
                  <td>{i.turno}</td>
                  <td>
                    <span className={`badge ${i.estaActivo ? 'activo' : 'vencido'}`}>
                      {i.estaActivo ? 'Activo' : 'Inactivo'}
                    </span>
                  </td>
                  <td>
          ...
          <div style={{ backgroundColor: '#2a2a2a', padding: '30px', borderRadius: '15px', width: '90%', maxWidth: '450px', border: '1px solid #aa3bff', color: 'white' }}>
            ...
            <form onSubmit={guardarInstructor} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div>
                ...
                <input type="text" value={nombreCompleto} onChange={e => setNombreCompleto(e.target.value)} required style={{ width: '95%', padding: '10px', borderRadius: '6px', border: '1px solid #555', backgroundColor: '#1a1a1a', color: 'white' }} />
              </div>

              <div>
                <label style={{ display: 'block', marginBottom: '5px' }}>Especialidad:</label>
                <input type="text" value={especialidad} onChange={e => setEspecialidad(e.target.value)} required placeholder="Ej. Boxeo, Zumba" style={{ width: '95%', padding: '10px', borderRadius: '6px', border: '1px solid #555', backgroundColor: '#1a1a1a', color: 'white' }} />
              </div>
